...
pub struct State<'p, B: Backend> {
    ...
    /// Warnings which have been generated by the backend (through
    /// `BackendResult`). Stored in a queue so that we can retrieve (and dismiss)
    /// the warnings in the order they were generated.
    ///
    /// This queue persists across backtracking.
    generated_warnings: RefCell<VecDeque<Warning<'p>>>,
}
...
#[derive(Clone, Debug)]
pub struct Warning<'p> {
    /// The warning message
    pub msg: String,
    /// The `Location` at which the warning was generated
    pub loc: Location<'p>,
    /// String representation of the backtrace at which the warning was generated
    pub backtrace: String,
    /// String representation of the LLVM path to where the warning was generated
    pub llvm_path: String,
    /// String representation of the source-language path to where the warning was generated
    pub src_path: String,
}
    ...
    B: 'p,
{
    ...
    pub fn new(project: &'p Project, start_loc: Location<'p>, mut config: Config<'p, B>) -> Self {
        ...
        let mut state = Self {
            ...
            generated_warnings: RefCell::new(VecDeque::new()),
    ...
    pub fn sat_with_extra_constraints<'b>(
        ...
        self.unwrap_backendresult(solver_utils::sat_with_extra_constraints(&self.solver, constraints))
    }
    ...
    pub fn bvs_must_be_equal(&self, a: &B::BV, b: &B::BV) -> Result<bool> {
        self.unwrap_backendresult(solver_utils::bvs_must_be_equal(&self.solver, a, b))
    }
    ...
    pub fn bvs_can_be_equal(&self, a: &B::BV, b: &B::BV) -> Result<bool> {
        self.unwrap_backendresult(solver_utils::bvs_can_be_equal(&self.solver, a, b))
    }
    ...
    /// Assert that the given `BV` equals 1. The `BV` must have bitwidth 1.
    pub fn assert(&self, bv: &B::BV) -> Result<()> {
        self.unwrap_backendresult(bv.assert())
    }
    ...
    /// For internal use: `state.read()` but without triggering watchpoints
    fn read_without_triggering_watchpoints(&self, addr: &B::BV, bits: u32) -> Result<B::BV> {
        match self.unwrap_backendresult(self.mem.borrow().read(addr, bits)) {
            Ok(val) => Ok(val),
    ...
    fn write_without_mut(&self, addr: &B::BV, val: B::BV) -> Result<()> {
        ...
        match self.unwrap_backendresult(result) {
            Ok(()) => (),
            e @ Err(Error::NullPointerDereference) => {
                if self.config.null_pointer_checking == NullPointerChecking::SplitPath {
                    // save a backtracking point to re-execute the current
            ...
            .get_triggered_watchpoints(addr, write_width)?
        {
            ...
            let new_value = self.read_without_triggering_watchpoints(&watchpoint_low, watchpoint_size_bits as u32)?;
            info!(
    ...
    /// Record the given warning message as being generated at the current
    /// location.
    ///
    /// This method uses interior mutability (only requiring `&self`) so that
    /// warnings can be recorded even in contexts where only immutable access to
    /// `State` is available.
    //
    // That means that the `State` methods which this calls need to be careful
    // not to generate warnings themselves.
    pub fn record_warning(&self, msg: String) {
        self.generated_warnings.borrow_mut().push_back(Warning {
            msg,
            loc: self.cur_loc.clone(),
            backtrace: self.pretty_backtrace(),
            llvm_path: self.pretty_path_llvm(),
            src_path: self.pretty_path_source(),
        })
    }

    /// Unwrap the `BackendResult` into a `Result`, recording the warning if
    /// there is one.
    pub fn unwrap_backendresult<T>(&self, backendresult: BackendResult<T>) -> Result<T> {
        for warning in backendresult.warnings {
            self.record_warning(warning);
        }
        backendresult.res
    }

    /// Get the oldest warning, and dismiss it.
    ///
    /// Returns `None` if there are no warnings remaining.
    pub fn get_and_dismiss_oldest_warning(&mut self) -> Option<Warning<'p>> {
        self.generated_warnings.borrow_mut().pop_front()
    }
...
mod tests {
    ...
    fn unsat() -> Result<()> {
        ...
        state.assert(&state.bv_from_bool(false))?;
    ...
    fn unsat_with_extra_constraints() -> Result<()> {
        ...
        let x = state.new_bv_with_name(Name::from("x"), 64)?;
        state.assert(&x.ugt(&state.bv_from_u64(3, 64)))?;
        assert_eq!(state.sat(), Ok(true));
    ...
    fn possible_solutions() -> Result<()> {
        ...
        // add x < 6 constraint
        state.assert(&x.ult(&state.bv_from_u64(6, 64)))?;
        ...
        // add x < 5 constraint
        state.assert(&x.ult(&state.bv_from_u64(5, 64)))?;
        ...
        // add x < 3 constraint
        state.assert(&x.ult(&state.bv_from_u64(3, 64)))?;
    ...
    fn backtracking() -> Result<()> {
        ...
        // assert x > 11
        let x = state.new_bv_with_name(Name::from("x"), 64)?;
        state.assert(&x.sgt(&state.bv_from_i64(11, 64)))?;
        ...
        state.assert(&x.slt(&state.bv_from_i64(8, 64)))?;
        assert_eq!(state.sat(), Ok(false));
    ...
    fn fork() {
        ...
        state.assert(&x.ult(&state.bv_from_u32(42, 64))).unwrap();
        ...
        state.assert(&x_1.ugt(&state.bv_from_u32(3, 64))).unwrap();
        state_2.assert(&x_2.ult(&state_2.bv_from_u32(3, 64))).unwrap();
