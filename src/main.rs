...
fn find_zero_of_func(ctx: &z3::Context, func: FunctionValue) -> Option<Vec<i32>> {
    ...
    let z3rval = symex_function(&mut state, func, &mut vars);
...
fn symex_binop<'ctx, F>(state: &State<'ctx>, inst: InstructionValue, vars: &mut VarMap<'ctx>, z3op: F)
    where F: FnOnce(&z3::Ast<'ctx>, &z3::Ast<'ctx>) -> z3::Ast<'ctx>
{
    ...
    let z3firstop = intval_to_ast(firstop.into_int_value(), state, vars);
    let z3secondop = intval_to_ast(secondop.into_int_value(), state, vars);
    state.assert(&z3dest._eq(&z3op(&z3firstop, &z3secondop)));
    vars.insert(inst.as_any_value_enum(), z3dest);
}

fn symex_icmp<'ctx>(state: &State<'ctx>, inst: InstructionValue, vars: &mut VarMap<'ctx>) {
    assert_eq!(inst.get_num_operands(), 2);
    let dest = get_dest_name(inst);
...
fn symex_return<'ctx>(state: &State<'ctx>, inst: InstructionValue, vars: &mut VarMap<'ctx>) -> z3::Ast<'ctx> {
    ...
    intval_to_ast(rval.into_int_value(), state, vars)
}
...
fn symex_br<'ctx>(state: &mut State<'ctx>, inst: InstructionValue, vars: &mut VarMap<'ctx>, cur_bb: BasicBlock) -> z3::Ast<'ctx> {
    match inst.get_num_operands() {
        1 => {
            ...
            symex_from_bb(state, bb, Some(cur_bb), vars)
        },
        3 => {
            // conditional branch
            let cond = inst.get_operand(0).unwrap().left().unwrap();
            assert!(cond.is_int_value());
            let z3cond = intval_to_ast(cond.into_int_value(), state, vars);
            ...
            if true_feasible && false_feasible {
                ...
                symex_from_bb(state, inst.get_operand(1).unwrap().right().unwrap(), Some(cur_bb), vars)
            } else if true_feasible {
                symex_from_bb(state, inst.get_operand(1).unwrap().right().unwrap(), Some(cur_bb), vars)
            } else if false_feasible {
                symex_from_bb(state, inst.get_operand(2).unwrap().right().unwrap(), Some(cur_bb), vars)
            } else if let Some((bb, prev_bb)) = state.revert_to_backtracking_point() {
                symex_from_bb(state, bb, Some(prev_bb), vars)
            } else {
...
fn symex_phi<'ctx>(state: &State<'ctx>, inst: InstructionValue, vars: &mut VarMap<'ctx>, prev_bb: Option<BasicBlock>) {
    ...
    let z3value = intval_to_ast(chosen_value.into_int_value(), state, vars);
...
fn intval_to_ast<'ctx>(v: IntValue, state: &State<'ctx>, vars: &VarMap<'ctx>) -> z3::Ast<'ctx> {
    if v.is_const() {
