...
fn no_args_nozero() {
    ...
    assert_eq!(find_zero_of_func(&ctx, func, &module, std::iter::empty(), Config::default()), None);
}
...
fn no_args_zero() {
    ...
    let args = find_zero_of_func(&ctx, func, &module, std::iter::empty(), Config::default()).expect("Failed to find zero of the function");
